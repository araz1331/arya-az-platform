...
- **Subscription Gating:** After login, `GET /api/subscription/check` verifies `has_chat`, `has_voice`, `has_sales` via the cross-service API at `sales.hirearya.com`. If `has_chat` is false, the dashboard shows a "Product Locked" overlay linking to `sales.hirearya.com/#pricing`.
- **Unified Sidebar Navigation:** Desktop sidebar (`ProductSidebar` component) and mobile hamburger menu (`MobileProductSwitcher`) connect 3 Replit projects: Chat (chat.hirearya.com), Voice (phonecall.hirearya.com), Sales (sales.hirearya.com). Lock/unlock based on subscription flags. SSO via Supabase access token passed as `?sso_token=` query parameter when navigating between projects.
- **Bundle Logic:** Bundle A = Chat + Voice ($169/mo), Ultimate = Chat + Voice + Sales ($599/mo). Bundle detection via `getBundleState()`. Upgrade modal shows context-aware messaging based on current bundle. Upsell buttons in sidebar and header for Bundle A users ("Add Sales Hunter +$430/mo") and single-product users ("Unlock Full Suite, Save 30%").
- **Stripe Bundle Webhook Mapping:** `server/webhookHandlers.ts` maps bundle price IDs to product flags (`has_chat`, `has_voice`, `has_sales`). Placeholder price IDs — update `BUNDLE_PRICE_IDS` with actual Stripe IDs when created. Syncs flags to centralized profiles table via cross-service API at `sales.hirearya.com/api/subscription/update`.
